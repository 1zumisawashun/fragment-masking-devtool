...
- **内容は厚いが見出し名が型と違う**（例: `## 背景・課題` はあるが `## 方針` が無い、
  タスクが `###` チェックリストでなく番号付きステップになっている、など） — 決定事項は
  既にほぼ揃っている可能性が高いので、grillingセッションを最初からやり直さず、まず
  「本当に未決定な論点だけ」を洗い出す軽いgrillingラウンドを回してから、`local-grill-me`の
  ステップ3（issue本文の下書き）で型に整形し直す。判定に迷う場合は黙って進めず、
  「内容はほぼ決まっているように見えるが、型を整えるためにgrillingを軽く回してよいか」を
  ユーザーに確認する。
